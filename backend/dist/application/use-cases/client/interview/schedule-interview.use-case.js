...
const calendarInvite_service_1 = require("../../../../infrastructure/external-service/calendarInvite.service");
class ScheduleInterviewUseCase {
    ...
    geminiSvc;
    mailSvc;
    constructor(candidateRepo, jobRepo, aiAnalysisRepo, interviewScheduleRepo, geminiSvc, mailSvc) {
    ...
    async execute(input) {
        ...
        if (!candidate.personal.email)
            throw new Error('Ứng viên chưa có email để gửi thư mời.');
        ...
        if (!analysis)
            throw new Error('Ứng viên chưa được AI phân tích. Vui lòng chạy phân tích trước khi đặt lịch.');
        ...
        if (!created)
            throw new Error('Không thể tạo lịch phỏng vấn.');
        const invite = (0, calendarInvite_service_1.buildInterviewCalendarInvite)({
...
//# sourceMappingURL=schedule-interview.use-case.js.map